...
const Entidade = mongoose.model("usuario");
...
router.post("/findbynome", (req, res) => {
  Entidade.find({ nome: { $regex: new RegExp(req.body.nome), $options: 'i' }}
    )
    .then((reg) => {
      if (reg.length == 0) {
        return res.send({ msg: "Nenhum registro encontrado!" });
...
router.post("/findbyautentica", (req, res) => {
  Entidade.findOne({ email: req.body.email, senha: req.body.senha })
    .then((reg) => {
      if (reg.length == 0) {
        return res.send({ msg: "Nenhum registro encontrado!" });
      }
